...
void main(int argc, char *argv[]) {
    ...
    printf("[P%d] grid id %d, coordinates (%d, %d)\n\n", Iam, id2D, coords2D[0], coords2D[1]);
    ...
    printf("[P%d] column id %d, coordinate (%d)\n\n", Iam, colID, coords1D[0]);

	/* la barrier assicura che tutti conoscano le proprie coordinate prima di assegnare aij*/
	MPI_Barrier(MPI_COMM_WORLD); 

	/* aij = (i+1)*10 + j + 1; 1 elemento matrice per ogni processore di posto (i,j) */  
	...
	for (i = 0; i <= nrow - 1; i++) { 
	 ...
	 /*gather in ogni colonna all’ultimo processo, che memorizza i dati ricevuti in alocal */  
